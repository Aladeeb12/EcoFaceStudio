...
# حل مشكلة المسار العربي في ميديا بايب قبل البدء
...
# --- 2. إعدادات الصفحة والتصميم المتطور ---
...
st.markdown("""
    ...
    .stMetric { background-color: rgba(255,255,255,0.05); padding: 10px; border-radius: 15px; }
    h1, h2, h3 { color: #d4e4bc !important; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
    .stButton>button {
        width: 100%;
    ...
    .stButton>button:hover { background-color: #d4e4bc !important; color: #1b3022 !important; }
    </style>
...
def get_models():
    ...
    seg_mod = mp_selfie_segmentation.SelfieSegmentation(model_selection=0) # 0 أسرع للكاميرا
    return mesh_mod, seg_mod

mesh_mod, seg_mod = get_models()

# --- 4. وظائف المعالجة الاحترافية ---
...
# إضافة خيار الكاميرا (لحل مشكلتك السابقة)
st.sidebar.subheader("⚙️ إعدادات النظام")
cam_id = st.sidebar.selectbox("مصدر الكاميرا", options=[0, 1, 2], index=0, help="جرب 1 إذا لم تظهر كاميرا الجهاز")
...
st.sidebar.subheader("🌻 فلاتر التجميل")
...
def process_frame(img_bgr):
    ...
    if res_mesh.multi_face_landmarks:
        ...
        acc_cfg = {
            "نظارة": (f_glasses, "assets/glasses.png", 168, 1.6, 0, c_glasses), 
            "شارب": (f_mustache, "assets/mustache.png", 2, 1.0, 20, c_mustache)
        }
...
if st.session_state.run_live:
    st_frame = st.empty()
    fps_text = st.empty()
    cap = cv2.VideoCapture(cam_id) # استخدام معرف الكاميرا المختار
    prev_time = 0
    ...
    while cap.isOpened() and st.session_state.run_live:
        ...
        # عرض الصورة باستخدام المعيار الجديد
    ...
    if file:
        ...
        with st.spinner("جاري تطبيق سحر الطبيعة..."):
            res_bgr = process_frame(orig_bgr)
            res_rgb = cv2.cvtColor(res_bgr, cv2.COLOR_BGR2RGB)
        
        c1, c2 = st.columns(2)
        with c1: st.image(img_in, caption="الصورة الأصلية", use_container_width=True)
        with c2: st.image(res_rgb, caption="النتيجة النهائية", use_container_width=True)
        
        buf = io.BytesIO()
        Image.fromarray(res_rgb).save(buf, format="PNG")
        st.download_button("💾 تحميل الصورة", buf.getvalue(), "eco_face.png", "image/png")